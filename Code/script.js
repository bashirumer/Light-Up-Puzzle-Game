...
function showRules() {
  // Set the display property to "block" for the rules popup and overlay
...
function hideRules() {
  // Set the display property to "none" for the rules popup and overlay
  document.getElementById("rulesPopup").style.display = "none";
  document.getElementById("overlay").style.display = "none";
}

// Reference to the game board element and an array to store its state
const boardElement = document.getElementById("board");
let boardState = [];

// preset puzzle for the entire game
const puzzles = [
  {
    walls: [
      [1, 1],
...
function initializeBoard() {
  // Loop to create a 10x10 grid of cells and initialize their state
  for (let i = 0; i < 10; i++) {
    boardState[i] = [];
    for (let j = 0; j < 10; j++) {
      // Set initial state to "empty" and create a cell element
...
function toggleCell(event) {
  // Extract row and column information from the clicked cell
...
function updateBoardView() {
  ...
  cells.forEach((cell, index) => {
    ...
    if (boardState[row][col] === "lit") {
      // Style the cell as a lit cell
      cell.style.backgroundColor = "#ffd700";

      cell.textContent = "";
    } else if (boardState[row][col] === "light") {
       // Style the cell as a light cell
      cell.style.backgroundColor = "#ffd700";
      cell.classList.add("light"); // Add light class
      cell.textContent = "";
    } else if (boardState[row][col] === "wall") {
      // Style the cell as a wall cell
...
function loadPuzzle(selectedPuzzle) {
  ...
  for (const wall of selectedPuzzle.walls) {
   ...
   // Place numbered walls on the board
...
// Resets the Board
function resetBoard() {
  // Iterate through the entire board and reset lit/light cells to empty
  for (let i = 0; i < 10; i++) {
    for (let j = 0; j < 10; j++) {
...
function refreshBoard() {
  // Iterate through the entire board and light up cells marked as light
  for (let i = 0; i < 10; i++) {
    for (let j = 0; j < 10; j++) {
...
// Function to hide the result overlay when clicking outside of it
